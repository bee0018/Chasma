...
const RemotePullRequestPage: React.FC<RemotePullRequestPageProps> = (props: RemotePullRequestPageProps) => {
    ...
    /** Gets or sets the selected change request assignees. */
    const [selectedAdditionalAssignees, setSelectedAdditionalAssignees] = useState<{ rowId: string, member?: RemoteProjectMember }[]>([]);

    /** Gets or sets the selected change request reviewers. */
    const [selectedReviewers, setSelectedReviewers] = useState<{ rowId: string, member?: RemoteProjectMember }[]>([]);
    ...
    const deleteGitLabReviewerRow = (rowId: string) => {
        const reviewers = selectedReviewers.filter(i => i.rowId !== rowId);
            ...
            {!successfullyCreated &&
                <>
                    ...
                    <div className="repository-actions">
                        <h3>{props.repository.hostPlatform === RemoteHostPlatform.GitLab ? "Additional " : ""} Assignees</h3>
                        <button
                    ...
                    {selectedReviewers.map(reviewerRow => (
