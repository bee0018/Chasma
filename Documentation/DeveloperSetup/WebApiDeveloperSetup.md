...
- The Web API will generate a typescript client that the frontend will use so it'll be easier to send requests and receive responses.
  ...
  - To make updates to objects that reflect in the frontend code, just rebuild the web API.

## Database
- We will be using Entity Framework Core for database operations. Upon startup of the application, it will perform a migration on the database. A new database will be created if one does not exist, or it will be updated if it does.
